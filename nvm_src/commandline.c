...
void PrintUsage()
{
    ...
    LOG_MSG("-pydSGMMode           0: Frame mode (default)\n");
    LOG_MSG("                      1: Level mode\n");
}
...
int ParseArgs(int argc, char *argv[], TestArgs *args)
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        if (argv[i][0] == '-')
        {
            ...
            else if (strcmp(&argv[i][1], "pydSGMMode") == 0)
            {
                if (!bDataAvailable || !sscanf(argv[++i], "%hhd", &args->pydSGMMode))
                {
                    LOG_ERR("ERR: -pydSGMMode should be followed by 0 or 1. \n");
                    return -1;
                }
            }
