...
)
{
    ...
    if (strcmp(epipolarFilename, ""))
    {
        ...
        LOG_INFO("GetEpipolarInfo: reading epipolar info file %s\n", epipolarFilename);
        ...
        {
            LOG_ERR("GetEpipolarInfo: Unable to open epipolar info file %s\n", epipolarFilename);
            return false;
        }
    }
    return true;
...
**/
...
#endif
    // For Pyramid SGM mode, set variable args.inputBuffering = 1 as this variable is used for registering, unregistering surf and submit logic as well
    args.inputBuffering = (args.pydSGMMode == NVMEDIA_IOFA_PYD_LEVEL_MODE) ? 1 : args.inputBuffering;
    // For Pyramid SGM mode set maxinputbuffering argument of Init API equal to number of pyammid levels
    status = NvMediaIOFAInit(testOFA, &ofaInitParams, (args.pydSGMMode == NVMEDIA_IOFA_PYD_LEVEL_MODE) ? ctx.pydLevel : args.inputBuffering);
    if (status != NVMEDIA_STATUS_OK)
    {
    ...
    while (true)
    {
           ...
           (ctx.frameRead > ctx.frameQueued))
        {
            ...
            if (args.pydSGMMode == NVMEDIA_IOFA_PYD_LEVEL_MODE)
            {
                for (k = ctx.pydLevel; k > 0; k--)
                {
                    memset(&ofaProcessParams, 0, sizeof(NvMediaIofaProcessParams));
                    ofaProcessParams.pydHintParams.pydHintMagnitudeScale2x = NVMEDIA_TRUE;
                    ofaProcessParams.currentPydLevel = k - 1U;
                    if (k < (ctx.pydLevel))
                    {
                        if (args.gridsize[k] >= args.gridsize[k-1])
                        {
                            ofaProcessParams.pydHintParams.pydHintWidth2x = NVMEDIA_TRUE;
                            ofaProcessParams.pydHintParams.pydHintHeight2x = NVMEDIA_TRUE;
                        }
                    }
                    status = NvMediaIOFAProcessFrame(testOFA, &surfArray, &ofaProcessParams, &epipolarInfo, k==1?&roiParams:NULL);
                    if (status != NVMEDIA_STATUS_OK)
                    {
                        LOG_ERR("NVMEDIA_OFA_MAIN: NvMediaIOFAProcessFrame failed for level %d \n", i);
                        status_main = -1;
                        goto fail;
                    }
                    LOG_INFO("NVMEDIA_OFA_MAIN: OFS estimation submitted for level = %d.\n", ofaProcessParams.currentPydLevel);
                }
            }
            else
            {
